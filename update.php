<?php
  ...
  if (@$_GET['q'] == 'addquiz' && $_SESSION['key'] == 'admin') {
    ...
    $good = $_POST['right'];
    $id = uniqid();
    $q3 = mysqli_query($con, "INSERT INTO quiz VALUES  ('$id', '$name', '$good', '$wrong', '$total' , NOW())");
  ...
  if(@$_GET['q']== 'quiz' && @$_GET['step']== 2) 
  {
    ...
    {
      $sn++;
      header("location:exercise.php?q=quiz&step=2&eid=$eid&n=$sn&t=$total")or die('Error152');
    }
    else if( $_SESSION['key']!='admin')
    {
      ...
      header("location:exercise.php?q=result&eid=$eid");
    }
    else
    {
      header("location:exercise.php?q=result&eid=$eid");
  ...
  if(@$_GET['q']== 'quizre' && @$_GET['step']== 25 ) 
  {
    ...
    $sun=$sun-$s;
    $q=mysqli_query($con,"UPDATE `rank` SET `score`=$sun ,time=NOW() WHERE email= '$email'")or die('Error174');
    header("location:exercise.php?q=quiz&step=2&eid=$eid&n=1&t=$t");
  }
?>
